...
class EditGoalExpandedSettings extends StatefulWidget {
  final titleController;

  const EditGoalExpandedSettings({ this.titleController});

  @override
  _EditGoalExpandedSettingsState createState() => _EditGoalExpandedSettingsState();
}

class _EditGoalExpandedSettingsState extends State<EditGoalExpandedSettings> {
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Text(
            'Согласно технологии SMART цель должна быть конкретной, измеримой, достижимой, значимой и ограниченной временем. Придерживаясь этих критериев вы поставите перед собой реальную цель, которую сможете выполнить.',
          ...
          TextFormField(
            ...
            onChanged: ((val) {
              if (val != '')
                global.store.set('Goal Title', val);
            }),
            validator: (value) {
          ...
          GestureDetector(
            onTap: ()async {
              var value = await showTopModalSheet<String>(context: context, child: 
                  ...
                  Wrap(
                      children: [
                        Container(decoration: BoxDecoration(
                          ...
                          child: TableCalenderWidget(func: function,))
                      ],
                    ));
